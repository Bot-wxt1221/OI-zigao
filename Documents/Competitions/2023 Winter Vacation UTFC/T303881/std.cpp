...
long long k1,b1,k2,b2,f_now[10010],f_last[10010],XQ[10010],YQ[10010],XP[10010],YP[10010],V[10010];
int Q[10010];

long long B(int i){return -V[i]-XQ[i]*XQ[i]-YQ[i]*YQ[i];}
long long K(int i){return XQ[i]<<1;}
long long X(int j){return XP[j]+k1*YP[j];}
long long Y(int j){return f_last[j]+XP[j]*XP[j]+YP[j]*YP[j]-(b1*YP[j]<<1);}
...
int main(){
    ...
    register long long ans=0x3f3f3f3f3f3f3f3fll;
    int n=read();
    int m=read();
    k1=read();
    b1=read();
